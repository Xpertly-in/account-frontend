...
export default function DynamicForm() {
  ...
  const handleProfileImageChange = async (file: File | null) => {
    ...
    try {
      ...
      // Update ca_profiles.profile_picture
      const { error: updateError } = await supabase
        .from('ca_profiles')
        .update({ profile_picture: publicUrl })
        .eq('user_id', auth.user.id);
      if (updateError) throw new Error('Failed to update profile image');
  ...
  const validateForm = (): boolean => {
    ...
    // Validate all steps
    onboardingFormConfig.sections.forEach((section: FormSection) => {
      const stepErrors = validateStep(onboardingFormConfig, onboardingFormConfig.sections.indexOf(section), formData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Update CA profile with form data
      const { error: updateError } = await supabase
        .from('ca_profiles')
        .update({
          ...formData,
    ...
    <div className="relative w-full max-w-7xl mx-auto px-4 py-8" ref={formRef}>
      ...
      <div className="md:hidden mb-8">
        <div className="flex items-center justify-center space-x-2">
          {onboardingFormConfig.sections.map((_: FormSection, index: number) => (
            <div
              key={index}
        ...
        <div className="mt-2 text-center">
          <p className="text-sm font-medium">{onboardingFormConfig.sections[0].title}</p>
          <p className="text-xs text-muted-foreground">{onboardingFormConfig.sections[0].subtitle}</p>
        </div>
      </div>
