...
import { Search, MapPin, Building2, Landmark, TreePine, Waves, Mountain, Sun, Star, Calendar, Award, Calculator, FileText, PieChart, TrendingUp, ArrowDown, Shield, Rocket } from "lucide-react";
import Image from "next/image";
import { CAService } from "@/services/ca.service";
import { FeaturedCA } from "@/types/ca.type";

const HeroSection = () => {
  const [selectedService, setSelectedService] = useState<SelectOption | null>(null);
  const [selectedLocation, setSelectedLocation] = useState<SelectOption | null>(null);
  const [featuredCAs, setFeaturedCAs] = useState<FeaturedCA[]>([]);
  ...
  const popularCities = [
    { name: "Mumbai", icon: Building2, color: "text-blue-600", landmark: "Gateway of India" },
    { name: "Delhi", icon: Landmark, color: "text-red-600", landmark: "Red Fort" },
    { name: "Bangalore", icon: TreePine, color: "text-green-600", landmark: "Silicon Valley" },
    { name: "Chennai", icon: Waves, color: "text-cyan-600", landmark: "Marina Beach" },
    { name: "Kolkata", icon: Building2, color: "text-yellow-600", landmark: "Victoria Memorial" },
    { name: "Pune", icon: Mountain, color: "text-purple-600", landmark: "Shaniwar Wada" },
    { name: "Hyderabad", icon: Sun, color: "text-orange-600", landmark: "Charminar" },
    { name: "Ahmedabad", icon: Building2, color: "text-pink-600", landmark: "Sabarmati Ashram" }
  ];
  ...
  const handleCAClick = (caId: number) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-base lg:text-lg text-gray-600 dark:text-gray-300 mb-8 leading-relaxed max-w-4xl">
              Expert <strong className="text-blue-700 dark:text-blue-300">GST Filing</strong>, <strong className="text-blue-800 dark:text-blue-300">Income Tax</strong>, <strong className="text-blue-900 dark:text-blue-300">Audit Support</strong> & More. 
              Connect with qualified CAs who understand your business and provide personalized financial solutions across India.
            </p>
            ...
            <div className="mb-8 max-w-4xl">
              ...
              {isLoadingCAs ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[1, 2, 3].map((index) => (
                    <div key={index} className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-blue-100 dark:border-gray-700 h-80 animate-pulse">
                    ...
                    >
                      ...
                      <div className="flex items-center mb-3">
                        <div className="flex items-center">
                          ...
                          <span className="ml-1 text-sm font-medium text-gray-900 dark:text-white">{ca.rating}</span>
                          <span className="ml-1 text-sm text-gray-500 dark:text-gray-400">({ca.reviews} reviews)</span>
                        </div>
                      </div>
                      
                      <div className="flex items-center mb-4">
                        <Calendar className="h-4 w-4 text-gray-400 mr-1" />
                        <span className="text-sm text-gray-600 dark:text-gray-400">{ca.experience} years experience</span>
                      </div>
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500 dark:text-gray-400">No featured CAs available at the moment.</p>
                </div>
              )}
            </div>
          ...
          <div className="lg:col-span-1 relative hidden lg:block">
            <div className="relative w-full h-96 flex items-center justify-center">
              ...
              <div className="relative w-full h-full">
                ...
                <div className="absolute top-4 left-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 animate-bounce" style={{animationDelay: '0.2s'}}>
                ...
                <div className="absolute top-20 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 animate-bounce" style={{animationDelay: '0.7s'}}>
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center">
                      <Calculator className="h-4 w-4 text-white" />
                    </div>
                    <span className="text-xs font-semibold text-gray-800 dark:text-white">ITR Filing</span>
                  </div>
                </div>

                <div className="absolute bottom-24 left-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 animate-bounce" style={{animationDelay: '1.2s'}}>
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-purple-600 rounded-lg flex items-center justify-center">
                      <Shield className="h-4 w-4 text-white" />
                    </div>
                    <span className="text-xs font-semibold text-gray-800 dark:text-white">Auditing</span>
                  </div>
                </div>

                <div className="absolute bottom-4 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 animate-bounce" style={{animationDelay: '1.7s'}}>
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-orange-500 rounded-lg flex items-center justify-center">
                      <TrendingUp className="h-4 w-4 text-white" />
                    </div>
                    <span className="text-xs font-semibold text-gray-800 dark:text-white">Investment</span>
                  </div>
                </div>

                <div className="absolute bottom-3 left-1/4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 animate-bounce" style={{animationDelay: '2.2s'}}>
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-pink-500 rounded-lg flex items-center justify-center">
                      <Rocket className="h-4 w-4 text-white" />
                    </div>
                    <span className="text-xs font-semibold text-gray-800 dark:text-white">Startup</span>
                  </div>
                </div>
              </div>
            </div>
