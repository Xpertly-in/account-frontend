 ...
 *
 * There are two ways to use these mocks:
 *
 * 1. RECOMMENDED APPROACH - Direct mocking in test files:
 *    - Import only the necessary helper functions (e.g., mockGlobalWindow)
 *    - Define your mocks at the top of your test file using jest.mock directly
 *    - Use require() to get references to the mocked functions after mocking
 *    - See useAnalytics.test.tsx, GoogleAnalytics.test.tsx, and AnalyticsOptOut.test.tsx for examples
 *
 * 2. Helper-based approach:
 *    - This approach may not work for complex mocks due to variable hoisting issues
 *    - Use only for simple mocks like localStorage or window objects
 *    - See the implementation of mockGlobalWindow for an example
 *
 * NOTE: For React component mocks, the helper-based approach does not work well
 * with TypeScript. Use the direct mocking approach instead.
 */
