...
describe("LoginForm Component", () => {
  ...
  test("handles Google sign-in click", async () => {
    ...
    expect(mocks.googleAuth.signIn).toHaveBeenCalled();
  });

  // Error handling tests
  test("displays error message for invalid credentials", async () => {
    const mocks = getMocks();
    mocks.auth.signIn.mockResolvedValueOnce({
  ...
  test("redirects to onboarding when onboarding not completed", async () => {
    ...
    mocks.supabase.single.mockResolvedValueOnce({
    ...
    await waitFor(() => {
      expect(mocks.toast.success).toHaveBeenCalledWith("Login successful", expect.anything());
      expect(mocks.router.push).toHaveBeenCalledWith("/ca/onboarding");
    });
  });
  ...
  test("redirects to dashboard when onboarding is completed", async () => {
    ...
    await waitFor(() => {
      expect(mocks.toast.success).toHaveBeenCalledWith("Login successful", expect.anything());
      expect(mocks.router.push).toHaveBeenCalledWith("/ca/dashboard");
    });
  });
  ...
  test("respects stored redirect path after login", async () => {
    ...
    await waitFor(() => {
      expect(mocks.router.push).toHaveBeenCalledWith("/ca/profile");
